...
var City = exports.City = function () {
	...
	_createClass(City, [{
		...
		key: "collectHeight",
		value: function collectHeight() {
			var collectH = {};
			var colH = this.colH;
			Array.from(colH).forEach(function (val, index) {
		...
		value: function bindEvent() {
			...
			// console.log(this)
			var collectH = this.collectH,
			    wrap = this.wrap;
			wrap.addEventListener("click", function (e) {
				var target = e.target;
				if (target.tagName == 'LI' && !target.classList.contains("currentCity")) {
					_this.obj.callback(target.innerHTML);
					_this.curCity.innerHTML = target.innerHTML;
					_this.hide();
				} else if (target.tagName == 'SPAN') {
					var _target = e.target;
					var top = collectH[_target.getAttribute("alpha")];
					_this.main.scrollTop = top;
				}
...
var _cites = __webpack_require__(3);
...
promise.then(function (datas) {
    var City_module = new _cites.City({
        data: datas,
    ...
    //点击入住地时
    liveArea.addEventListener("click", function () {
        City_module.show();
    }, false);
});
...
var date = new Date(),
    ...
    cHour = date.getHours();
if (cHour > 18) {
    var newDate = new Date(_cYear, _cMonth, _cDay + 1),
        _cYear = newDate.getFullYear(),
        _cMonth = newDate.getMonth() + 1,
        _cDay = newDate.getDate();
}

currentData.innerHTML = cYear + "-" + cMonth + "-" + cDay;
//设置离店日期
var Ldate = new Date(cYear, cMonth, cDay + 2),
...
var calendar = new _calendar.Calendar({
    ...
    callback: function callback(checkin, checkout, num) {
...
var selectTtime = document.querySelectorAll(".selectTtime");
Array.from(selectTtime).forEach(function (val, index) {
    selectTtime[index].onclick = function () {
        calendar.show();
    };
});
...
document.querySelector(".ps").onclick = function () {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function (position) {
            ...
            //跨域请求
            (0, _Ajax.jsonp)("http://apis.map.qq.com/ws/location/v1/ip?ip=169.254.207.187&output=jsonp&key=QZFBZ-2F7AD-FRI4M-PDHF5-KU2W6-D3F7J", function (data) {
